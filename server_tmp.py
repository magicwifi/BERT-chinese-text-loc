...
def predict_online(sentence):
    ...
    with graph.as_default():
        ...
        print(pred_label_result)
            #todo: 组合策略
        person, loc, org = strage_combined_link_org_loc(sentence, pred_label_result[0])
        print('time used: {} sec'.format((datetime.now() - start).total_seconds()))
        return person, loc, org
...
def strage_combined_link_org_loc(tokens, tags):
    ...
    #loc = return_output(loc)
    #person = return_output(person)
    #org = return_output(org)
...
def api_url(subpath):
  ...
  person, loc, org =predict_online(title)
  prediction = client.predict_title(title)
  print(prediction)

  title_dict = [{'prediction':prediction,'title':title,'person':person,'loc':loc,'org':org}]

  return "successCallback"+"("+json.dumps(title_dict,ensure_ascii=False)+")"
...
def api_article(subpath):
  ...
  person, loc, org =predict_online(article)
  #print(prediction)
  prediction = client.predict_article(article)

  article_dict = [{'prediction':prediction,'article':article,'org':org}]
  return "successCallback"+"("+json.dumps(article_dict,ensure_ascii=False)+")"
